...
//subscribe回调函数
void pointCvt(const sensor_msgs::PointCloud2::ConstPtr& rosPoint);
...
int main(int argc, char **argv) {
  ...
  // private_nh.param("file_directory", file_directory, std::string("/home/zzj/My_ws/map_pcd2jpg/"));
  // private_nh.param("file_name", file_name, std::string("scans1"));
  ...
  private_nh.param("thres_point_count", thres_point_count, 6);


  
  private_nh.param("pointcloud_topic_name", pointcloud_topic_name, std::string("/pointcloud"));
  private_nh.param("map_topic_name", map_topic_name, std::string("map"));


// My revise
  ros::Subscriber subPoint = nh.subscribe<sensor_msgs::PointCloud2>(pointcloud_topic_name, 5, pointCvt);//订阅来自算法处理完后的点云地图数据，回调函数进行处理这些点云数据

  map_topic_pub = nh.advertise<nav_msgs::OccupancyGrid>(map_topic_name, 1);
...
void SetMapTopicMsg(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  // origin的确定
  msg.info.origin.position.x = x_min;  //左下角的格子对应的物理坐标系 [m,m,rad]
  msg.info.origin.position.y = y_min;
  msg.info.origin.position.z = 0.0;  //这个是确定栅格地图的水平高度是多少
...
//算法处理完的点云数据进来处理
void pointCvt(const sensor_msgs::PointCloud2::ConstPtr& rosPoint) //算法处理完的点云数据进来处理
{
  ////pcl::PCLPointCloud2 pcl_pc2;
  ////sensor_msgs::PointCloud2 pl2 = *rosPoint;
  pcl::fromROSMsg(*rosPoint, *pcd_cloud);
  ////pcl_conversions::moveToPCL(pl2,pcl_pc2);//格式转化，将sensor_msgs::PointCloud2转化为pcl::PCLPointCloud2
  ////pcl::fromPCLPointCloud2(pcl_pc2, *pcd_cloud);

  std::cout << "-- pointCvt: 点云数据点数" << pcd_cloud->points.size()<<std::endl;
  ...
  //SetMapTopicMsg(pcd_cloud,map_topic_msg);//转栅格
}
