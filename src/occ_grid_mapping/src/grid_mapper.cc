...
void GridMapper::updateMap (const sensor_msgs::PointCloud2ConstPtr& cloud_msg, geometry_msgs::Transform& transform) 
{
    ...
    for (size_t i = 0; i < ranges.size(); ++i) {
        ...
        // 高斯分布更新
        updateGridWithGaussian(p_w, cell_size);
    }
}
...
void GridMapper::updateGridWithGaussian(const Eigen::Vector2d& p_w, double cell_size) {
    
    // 高斯分布更新
    // 2m 对应 1grid(0.05 m) = 2标准差
    const double sigma = p_w.norm() * (cell_size / 2 / 2.0); // 高斯分布的标准差
